...
public class SolicitationServiceImpl implements SolicitationService {
   ...
   // @Inject private SolicitationPrinter solicitationPrinter;
    ...
    public List<Object[]> findIndividualPagination(String sSearch, int iDisplayStart, int iDisplayLength) {
        ...
        if (getUserLogged().getRole().getId() <= 7){
            solicitations = solicitationDAO.findPaginationIndividualCoordinator(search, iDisplayStart, iDisplayLength, getUserLogged().getCostCenters());
        }else{
            solicitations = solicitationDAO.findPaginationIndividual(search, iDisplayStart, iDisplayLength, getUserLogged());
        }
        List<Object[]> solicitationsList = new ArrayList<>();
        for (Solicitation solicitation : solicitations){
            String colCode = solicitation.getId().toString() ;
            ...
            String buttonCancel = "<a onclick=cancelSolicitation("+solicitation.getId()+")><span class=\"fa fa-trash-o btn btn-default btn-xs\"></span></a>";
            String [] row = {
                    colCode,
                    colInitialDate,
                    colCostCenter,
    ...
    public List<Object[]> findMissingPagination(String sSearch, int iDisplayStart, int iDisplayLength) {
        ...
        if (getUserLogged().getRole().getId() == 5 || getUserLogged().getRole().getId() == 7  ){
            solicitations = solicitationDAO.findMissingPaginationAnalyst(search, iDisplayStart, iDisplayLength, getUserLogged().getCostCenters());
        }else if (getUserLogged().getRole().getId() <= 6 && getUserLogged().getRole().getId() != 5){
            solicitations = solicitationDAO.findMissingPaginationCoordinator(search, iDisplayStart, iDisplayLength, getUserLogged().getCostCenters());
        }
        List<Object[]> solicitationsList = new ArrayList<>();
        for (Solicitation solicitation : solicitations){
            String colCode = solicitation.getId().toString() ;
